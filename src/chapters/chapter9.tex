...
\end{equation}
The informed schedule knows $s^*$ exactly and needs to be slow only in the crossing window of width $O(\Delta_*)$, giving $T_{\mathrm{inf}} = \Theta(\Delta_*/v_{\mathrm{slow}})$ by \autoref{thm:aqo-runtime} applied to any gap profile in the class (here $T_{\mathrm{inf}}$ is the informed runtime for the rank-one gap profile, which has linear approach $\alpha = 1$ near $s^*$; the crossing window width is $\delta_s = \hat{g}/c_L = \Theta(\Delta_*)$ by \autoref{eq:gmin-deltas-relation}, giving $T_{\mathrm{inf}} = \Theta(\delta_s/v_{\mathrm{slow}}) = \Theta(\Delta_*/v_{\mathrm{slow}})$ via \autoref{thm:aqo-runtime}). The velocity factors cancel:
\begin{equation}
...
Group the $\lceil n/2 \rceil$ iterations by the distance $d_i$ in dyadic shells. For any fixed $s^*$, at most $O(1)$ binary search midpoints fall in each shell $d_i \in [2^{-j-1}, 2^{-j}]$ (binary search halves the search interval at each step, so consecutive distances satisfy $d_{i+1} \geq d_i/2$; hence at most two midpoints can have distances in any fixed dyadic interval $[2^{-j-1}, 2^{-j}]$).
...
\begin{proof}
The substitution $u = (c|s-s^*|^\alpha)/\Delta_*$ transforms the integrand near $s^*$ to $\Delta_*^{-\beta}(1+u)^{-\beta} \cdot \alpha^{-1}(\Delta_*/c)^{1/\alpha}\, u^{1/\alpha - 1}\,du$, giving a factor $\Delta_*^{1/\alpha - \beta}$ times a convergent integral (convergent as $u \to \infty$ iff $\beta > 1/\alpha$, which is ensured by the hypothesis, and at $u = 0$ for all $\alpha > 0$). The contribution from outside a neighborhood of $s^*$ is $O(1)$: for $|s - s^*| \geq \delta$ with $\delta$ fixed, $g(s) \geq g_0 > 0$ for a constant $g_0$ independent of $\Delta_*$, so $\int_{|s-s^*| \geq \delta} g(s)^{-\beta}\,ds \leq g_0^{-\beta}$.
\end{proof}
...
The quantum query complexity of $A_1$ estimation at precision $\varepsilon$ is $\Theta(1/\varepsilon)$. The lower bound is achieved on two-level instances with $\Delta_1 = 1$.
\end{theorem}
...
The generic extrapolation barrier (\autoref{thm:generic-barrier}) shows that the interpolation breakdown at precision $2^{-n/2}$ is not an artifact of a specific construction. Any polynomial extrapolation scheme with $d = \mathrm{poly}(n)$ nodes faces Lebesgue amplification $\Lambda_d(x^*) \geq 2^{d-1}$ (provided $x^*$ lies at distance at least $b - a$ from the sample interval $[a, b]$, which holds at the schedule-relevant precision since $x^* = \Theta(2^{-n/2})$ and the interpolation nodes lie in $[0, 1/\mathrm{poly}(n)]$), so the entire class of polynomial-interpolation reductions for $A_1$ requires precision $2^{-\Omega(n)}$.
