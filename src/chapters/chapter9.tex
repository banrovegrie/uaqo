...
$\det(I_k - (1-s)G(\lambda,s)) = 0$, where
$G(\lambda,s) = U^\dagger(sH_z - \lambda I)^{-1}U$.
On the two-level family ($E_0 = 0$, $E_1 = \Delta$), this reduces to
$\det(I_k - (x/\Delta)B) = 0$, with
$B = U_{\mathrm{exc}}^\dagger U_{\mathrm{exc}}$ and $x = (1-s)/s$.
