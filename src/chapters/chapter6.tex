...
Chapter 5 established the crossing window $\mathcal{I}_{s^*}$ where the spectral gap satisfies $g(s) = \Theta(g_{\min})$, and stated two bounds for the regions outside: a linear lower bound to the left (\autoref{lem:gap-left-preview}) and a linear lower bound to the right (\autoref{lem:gap-right-preview}). The complete gap profile determines the runtime of the adiabatic algorithm through the integral $\int_0^1 g(s)^{-2}\, ds$. This chapter proves both lemmas.
...
\end{equation}
This weighting arises naturally in first-order perturbation theory: the correction to the ground state of $sH_z$ due to the perturbation $-(1-s)\ket{\psi_0}\bra{\psi_0}$ has exactly this form. Normalization is immediate:
\begin{equation}
...
\begin{equation}
g(s^* - \delta_s) \geq \frac{A_1(A_1+1)}{A_2}\cdot\delta_s = \hat{g} \geq \frac{g_{\min}}{1 - 2\eta},
\end{equation}
using $A_1(A_1+1)\delta_s/A_2 = \hat{g}$ from Eq.~\eqref{eq:gmin-deltas-relation} (equivalently, $A_1 = s^*(A_1+1)$) and $g_{\min} \geq (1-2\eta)\hat{g}$ from Eq.~\eqref{eq:gmin-formula}. The gap at the window boundary exceeds $g_{\min}$, confirming that the minimum gap lies within $\mathcal{I}_{s^*}$.

The same bound follows from concavity. The second derivative of $\lambda_0(s)$ satisfies

$$\ddot{\lambda}_0(s) = -2\sum_{j \geq 1} |\bra{\phi_j(s)}\dot{H}\ket{\phi_0(s)}|^2/(E_j(s) - E_0(s)) \leq 0$$

where $\dot{H} = H_z + \ket{\psi_0}\bra{\psi_0}$ and $\ket{\phi_j(s)}$ are the instantaneous eigenstates. The tangent to the concave function $\lambda_0(s)$ at $s = s^*$ lies above it everywhere, and its slope evaluated using Eq.~\eqref{eq:lambda0-upper} reproduces the same linear bound.
...
\end{equation}
Bounding the gap from below reduces to bounding the resolvent norm from above.

The adiabatic Hamiltonian $H(s) = sH_z - (1-s)\ket{\psi_0}\bra{\psi_0}$ is a rank-one perturbation of $sH_z$. Rank-one perturbations have a classical inversion formula. The Sherman-Morrison identity \cite{sherman_morrison} states that for an invertible operator $A$ and vectors $\ket{u}, \bra{v}$,
\begin{equation}
...
\label{lem:gap-right}
Let $k = 1/4$, $a = 4k^2\Delta/3 = \Delta/12$, and $s_0$ as in Eq.~\eqref{eq:s0-definition}. Then for all $s \geq s^*$, the spectral gap of $H(s)$ satisfies
\begin{equation}
\label{eq:gap-right}
...
We bound the numerator and denominator of the second term separately. Both require that $\beta(s) \leq s(E_k - E_0)/2$ for all $k \geq 1$, which ensures the Taylor expansion in powers of $\beta/(s(E_k - E_0))$ converges rapidly. Since $\beta(s) \leq a = \Delta/12$ and $s(E_k - E_0) \geq s^*\Delta \geq \Delta/3$ (using $s^* = A_1/(A_1+1) \geq 1/3$, which holds when $A_1 \geq 1/2$, i.e., $d_0 \leq N/2$), we have $\beta \leq \Delta/12 < \Delta/6 \leq s(E_k - E_0)/2$. When $d_0 > N/2$, the majority of states are ground states and random sampling finds a solution with constant probability, making the adiabatic approach unnecessary.
...
\end{equation}
where, writing $\epsilon_0 = d_0/N$,
\begin{equation}
\label{eq:f-definition}
f(s) = \frac{\epsilon_0\, s^2(1-s) + 4A_2\beta^2(1-s)}{\epsilon_0\, s^2(1-s) + \beta\, s\,\dfrac{s - s^*}{1 - s^*} - 2A_2\beta^2(1-s)}.
\end{equation}
...
\begin{equation}
1 - \frac{(1-s)A_1}{s} + \frac{(1-s)\epsilon_0}{\beta} = \frac{s - s^*}{s(1-s^*)} + \frac{(1-s)\epsilon_0}{\beta},
\end{equation}
since $1 - A_1(1-s)/s = (s - A_1(1-s))/s = (s - s^*(1-s)/(1-s^*))/s = (s(1-s^*) - s^*(1-s))/(s(1-s^*)) = (s-s^*)/(s(1-s^*))$. Multiplying through by $\beta s^2(1-s)$ and collecting the Taylor-bounded terms into the $A_2\beta^2$ contributions gives Eq.~\eqref{eq:f-definition}.

The numerator of $f(s)$ measures the rank-one perturbation's effect on the resolvent: the $\epsilon_0$ term comes from the $\ket{0}$ component of $\ket{\psi_0}$ (the ground-state overlap), while the $A_2$ term comes from the excited components. The denominator captures the spectral rigidity: the term $\beta\,s(s-s^*)/(1-s^*)$ grows as $\gamma$ moves away from the crossing, stabilizing the resolvent against the perturbation. Near $s^*$, the denominator is small (the gap is small), so $f(s^*)$ is $O(1)$. As $s$ increases, the denominator grows and $f(s) \to 0$.
...
\textbf{Monotonicity of $f$.} We show $f'(s) < 0$ for $s \in [s^*, 1]$. Write $f = u/v$ with
\begin{align}
\label{eq:uv-def}
u &= \epsilon_0\,s^2(1-s) + 4A_2\beta^2(1-s), \\
v &= \epsilon_0\,s^2(1-s) + \beta\,s\,\frac{s - s^*}{1-s^*} - 2A_2\beta^2(1-s). \nonumber
\end{align}
Then $f' = (u'v - uv')/v^2$, so the sign of $f'$ is determined by $u'v - uv'$.

Computing $u'$ and $v'$ using $\beta' = a/(1-s_0)$:
\begin{align}
u' &= \frac{4aA_2\beta}{1-s_0}(2 + s_0 - 3s) + \epsilon_0\, s(2 - 3s), \label{eq:u-prime} \\
v' &= \frac{a(3s^2 - 2s(s^* + s_0) + s^*s_0)}{(1-s_0)(1-s^*)} - \frac{2aA_2\beta}{1-s_0}(2 + s_0 - 3s) + \epsilon_0\,s(2-3s). \nonumber
\end{align}

Expanding $u'v$ and $uv'$ and taking the difference, two terms cancel exactly: the $\epsilon_0^2 s^3(2-3s)(1-s)$ term and the $8a A_2^2\beta^3(1-s)(2+s_0-3s)/(1-s_0)$ term. The remaining expression has three terms \cite{braida2024unstructured}:
\begin{align}
\label{eq:upv-uvp}
u'v - uv' =\;& -\frac{4aA_2\beta^2}{(1-s_0)(1-s^*)}\Big(s^2(1 + s_0 - s^*) - 2s\,s_0 + s^*s_0\Big) \\
&+ \frac{12aA_2\epsilon_0\,\beta}{1-s_0}\,s(1-s)^2\,s_0 \nonumber \\
&- \frac{\epsilon_0\,s^2 a}{(1-s_0)(1-s^*)}\Big({-}s^2(s^* + s_0 - 1) + 2s\,s_0 s^* - s^*s_0\Big). \nonumber
\end{align}
The first and third terms are negative; the second is positive (it is the only term involving $\epsilon_0\,s_0$, which arises from the shift of $s_0$ below $s^*$). We must show the first negative term dominates the positive one.

Factor out $-4aA_2\beta/(1-s_0)$ from the sum of the first two terms:
\begin{equation}
\label{eq:domination-argument}
-\frac{4aA_2\beta}{1-s_0}\left(\frac{\beta}{1-s^*}\Big(s^2(1+s_0-s^*) - 2s\,s_0 + s^*s_0\Big) - 3\epsilon_0\,s_0\,s(1-s)^2\right).
\end{equation}
The quadratic $s^2(1+s_0-s^*) - 2s\,s_0 + s^*s_0$ is a convex function of $s$ that is minimized at some $s_m < s^*$ and is positive for $s \geq s^*$. The cubic $s(1-s)^2$ is maximized at $s = 1/3 \leq s^*$. Therefore, on $[s^*, 1]$, the bracket in~\eqref{eq:domination-argument} is bounded below by its value at $s = s^*$:
\begin{equation}
\frac{a(s^*-s_0)^2}{1-s_0} - 3\epsilon_0\, s_0(1-s^*)^2.
\end{equation}
...
\begin{equation}
f(s^*) = \frac{\epsilon_0\,{s^*}^2(1-s^*) + 4A_2 k^2 g_{\min}^2(1-s^*)}{\epsilon_0\,{s^*}^2(1-s^*) - 2A_2 k^2 g_{\min}^2(1-s^*)}.
\end{equation}
Using $g_{\min} \approx (2s^*/\sqrt{A_2})\sqrt{\epsilon_0}$ (from Eq.~\eqref{eq:gmin-formula} with $\hat{g} = 2s^*\sqrt{\epsilon_0/A_2}$), we have $A_2 k^2 g_{\min}^2 \approx 4k^2{s^*}^2\epsilon_0$. Substituting:
\begin{equation}
\label{eq:f-at-sstar}
...
For the running example ($M = 2$, $\Delta = 1$), the bound gives $g(s) \geq (1/30)(s - s_0)/(1 - s_0)$, where $s_0 = 1/2 - O(1/\sqrt{N})$ is close to $s^* \approx 1/2$ for large $N$. Near $s = 3/4$, the exact gap from Eq.~\eqref{eq:grover-gap} is $g(3/4) = \sqrt{1/4 + 3/(16N)} \approx 1/2$, while the bound gives approximately $(1/30)(1/4)/(1/2) = 1/60$. The bound is conservative by a factor of approximately $30$ but correctly captures the linear growth. This constant is the price of a clean, uniform bound valid for all problem Hamiltonians satisfying the spectral condition.
...
\begin{equation}
\frac{A_1(A_1+1)}{A_2}\cdot\delta_s = \hat{g} \geq \frac{g_{\min}}{1 - 2\eta} > g_{\min},
\end{equation}
so the left bound exceeds the window bound. At $s = s^*$, the right bound gives $g(s^*) \geq 2\beta(s^*)/(1 + f(s^*)) = 2k\,g_{\min}/5 = g_{\min}/10$, which is below $g_{\min}$ by a constant factor but still $O(g_{\min})$. The window bound provides the tighter estimate $g(s^*) = g_{\min}$.

The gap profile has a characteristic shape. It forms a broad V centered at $s^*$, with a narrow rounded minimum of width approximately $2\delta_s$. The left arm has slope $A_1(A_1+1)/A_2$, which is $O(\mathrm{poly}(n))$ for Ising Hamiltonians. The right arm has the shallower slope $\Delta/(30(1-s_0))$, controlled by the spectral gap $\Delta$ of the problem Hamiltonian. The asymmetry in the bounds --- steep on the left, shallow on the right --- is a proof artifact: the variational bound captures the true slope closely, while the resolvent bound sacrifices tightness for uniform validity across a more complicated spectral landscape. At the endpoints, $g(0) = 1$ (the initial gap between eigenvalues $-1$ and $0$ of $H_0$) and $g(1) = \Delta$ (the gap of $H_z$).
...
\begin{tikzpicture}[scale=1.0, >=stealth]
  ...
  % Exact gap (schematic smooth curve)
  ...
  \node[blue!70!black, right] at (10.1, 4.0) {$g(s)$};

  % Lower bounds (dashed)
  % Left: linear with slope A1(A1+1)/A2, passes through (s*, 0) extended
  ...
  % Right: linear from near s* upward
  \draw[dashed, thick, purple!70!black] (\sstar, {2*0.25*\gmin/3}) -- (10, {(10-\sstar)*0.13+2*0.25*\gmin/3});

  % Vertical dashed lines at s*-ds, s*, s*+ds
...
\end{tikzpicture}
\caption{Schematic gap profile for $H(s)$. The solid curve shows the true spectral gap $g(s)$, which dips to $g_{\min}$ at $s = s^*$ and rises on both sides. The dashed lines show the piecewise lower bounds: linear on the left (steep), constant in the window, and linear on the right (shallower). The three regions $\mathcal{I}_{s^\leftarrow}$, $\mathcal{I}_{s^*}$, $\mathcal{I}_{s^\rightarrow}$ are indicated.}
...
For the running example, the exact gap $g(s) = \sqrt{(2s-1)^2 + 4s(1-s)/N}$ and the piecewise bound from \autoref{thm:complete-profile} can be compared directly. The left bound has slope approximately $2$, matching the exact slope $|g'(s^*)| = 2\sqrt{1 - 1/N} \approx 2$. The window bound $g_{\min} = 1/\sqrt{N}$ is exact. The right bound has slope approximately $1/15$ near $s^*$, weaker than the true slope by a factor of $30$, but sufficient for the runtime integral since the window dominates.
...
\end{equation}
which is $O(1/(C^2\delta_s))$. In the window, $g(s) \geq g_{\min}$ gives $\int_{s^*-\delta_s}^{s^*+\delta_s} g(s)^{-2}\,ds \leq 2\delta_s/g_{\min}^2$. The window contribution $\delta_s/g_{\min}^2 = \Theta(A_2^{3/2}/A_1^2 \cdot \sqrt{N/d_0})$ dominates the outer regions, and the full integral --- including the $\Delta$-dependent right-arm contribution --- yields the runtime $T = O((\sqrt{A_2}/(A_1^2\Delta^2))\sqrt{N/d_0})$ that Chapter 7 derives rigorously.
