...
Chapter 7. The profile is steep on the left, flatter on the right, and nearly
...
three terms remain. Two are negative. One is positive and proportional to
$(d_0/N)\,s_0$, which appears because the line origin is shifted below $s^*$.
The real work is to show that the negative terms dominate this positive
contribution on all of $[s^*,1]$.
Write $f = u/v$ with
\begin{align}
\label{eq:uv-def}
...
\end{align}
The first and third terms are negative, while the middle term is positive. That
middle term is the only one that contains $(d_0/N)\,s_0$, so it is exactly the
cost of shifting the line origin below $s^*$. We now show that the first
negative term controls it.
