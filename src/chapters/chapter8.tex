...
\end{equation}
where $\Lambda_M(x^*)$ is the Lebesgue function at $x^*$. For the odd-integer nodes $x_j = 2j+1$ and evaluation point $x^* = -2\Delta_k \leq 0$ (outside the interval $[1, 2M-1]$): each numerator factor $|x^* - x_i| = 2\Delta_k + 2i + 1 \leq 2B + 1$. For the denominator, $\prod_{i \neq j}|x_j - x_i| = \prod_{i \neq j} 2|j-i| = 2^{M-1}\, j!\,(M-1-j)!$, since the nodes are equally spaced with spacing $2$. The sum over $j$ evaluates to
\begin{equation}
...
\]
Even for $c = 1$ (the most favorable case $M = n$), the exponent $n/2 + \Omega(n\log n)$ diverges. The upper bound on the degeneracy error already exceeds $1/2$, so the rounding step cannot be guaranteed to succeed.
\end{proof}
