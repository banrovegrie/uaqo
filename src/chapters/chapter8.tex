...
The barrier is not an artifact of the paper's specific construction. Any scheme that extracts integer degeneracies by evaluating a polynomial outside its interpolation interval faces exponential amplification. For the equispaced nodes used in our construction, the Lebesgue function $\Lambda_d(x^*)$ satisfies $\Lambda_d(x^*) \geq 2^{d-1}$ whenever $x^*$ lies at distance at least $|b - a|$ from the interpolation interval $[a,b]$. No rearrangement of interpolation nodes, no alternative polynomial basis, and no change of variables can circumvent this. The same structural obstacle appears in quantum computational advantage proposals: the polynomial interpolation techniques used to prove hardness of boson sampling \cite{aaronson2011bosonsampling} and random circuit sampling \cite{bouland2019complexity} face analogous amplification barriers when extending hardness from exponentially small to moderate error regimes. In each case, a worst-case hardness result extends to average-case via polynomial interpolation, but the Lebesgue constant of the interpolation scheme prevents the argument from reaching constant or inverse-polynomial error tolerances --- precisely the regime needed for practical claims of computational advantage.
