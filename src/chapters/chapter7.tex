...
error, which otherwise recreates the usual $O(1/g_{\min}^2)$ behavior.
...
Applying \autoref{thm:adaptive-rate} to $H(s)=-(1-s)\ket{\psi_0}\bra{\psi_0}+sH_z$ with the gap profile of \autoref{thm:complete-profile} requires three concrete steps. We construct a continuous lower bound $g_0(s)$ from the piecewise bounds, compute $B_1$ and $B_2$ by splitting $\int g_0^{-p}\,ds$ across the three regions and showing each contributes $O(\text{poly}(n)\cdot g_{\min}^{1-p})$, and then evaluate the constant $c$.
...
\begin{remark}[$A_1^2$ vs $A_1(A_1+1)$]
\label{rem:A1-squared}
Our published work \cite{braida2024unstructured} states $A_1^2$ in Theorem~1. The expression $A_1(A_1+1)$ follows from the proof derivation in Appendix~A-IV of \cite{braida2024unstructured}, and is what the algebra above produces. For Ising Hamiltonians with $A_1 = O(\mathrm{poly}(n))$, the distinction is absorbed by $O(\cdot)$ notation, since $A_1(A_1+1) = A_1^2 + A_1 = \Theta(A_1^2)$.
\end{remark}
...
Compared with the constant-rate theorem, the key gain is here. Time
