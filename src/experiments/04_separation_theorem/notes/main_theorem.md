...
Consider the adiabatic evolution $H(s) = (1-s)H_0 + sH_1$ with spectral gap $\Delta(s)$ having minimum $\Delta^*$ at unknown position $s^* \in [s_L, s_R]$.

Let:
- $T_{\text{inf}}(\Delta)$ = runtime of optimal gap-informed schedule for gap function $\Delta$
- $T_{\text{unf}}$ = runtime of any gap-uninformed schedule achieving same error
...
where $\delta_s = O(\Delta^*)$ is the crossing width.
...
- $s_R - s_L = \Theta(1)$ (constant uncertainty in $A_1$ range)
- $\delta_s = \Theta(2^{-n/2})$

Therefore: $T_{\text{unf}} / T_{\text{inf}} = \Omega(2^{n/2})$
...
1. **Adversarial argument**: For any schedule u(s), consider $s' \in [s_L, s_R]$ where |u'(s')| is largest.
...
3. **Error bound**: By Jansen-Ruskai-Seiler, error near crossing scales as:
   ```
   eta approx (u'(s*))^2 * delta_s / Delta*^3
   ```

4. **Contradiction**: If u is "fast" anywhere in [s_L, s_R], adversary places gap there, causing large error.

5. **Conclusion**: u must be "slow" throughout $[s_L, s_R]$, giving $T \geq \Omega((s_R - s_L) / \Delta^{*2})$.


### Upper Bound (uniform slow achieves this)

1. **Construction**: $u'(s) = v_{\text{slow}}$ for $s \in [s_L, s_R]$, fast elsewhere.
...
4. **Time**: $T = (s_R - s_L) / v_{\text{slow}} + O(1) = O((s_R - s_L) / \Delta^{*2})$.


### Comparison

- Gap-informed: $T_{\text{inf}} = O(\delta_s / \Delta^{*2}) = O(1/\Delta^*)$
- Gap-uninformed: $T_{\text{unf}} = O((s_R - s_L) / \Delta^{*2})$
- Ratio: $(s_R - s_L) / \delta_s$
